...
class MaskChooser:
    ...
    def __write_commands(self, mask_image):
        cv2.putText(
            mask_image,
            f"Premi '{self.CHANGE_MASK}' cambiare mask",
            (20, 20),
        ...
        cv2.putText(
            mask_image,
            f"Premi '{self.SELECT_MASK}' per selezionare la mask e proseguire",
            (20, 45),
            cv2.FONT_HERSHEY_DUPLEX,
            .7,
            (255, 0, 0),
    ...
    def choose_mask(self):
        while True:
            ...
            if key == ord(self.CHANGE_MASK) or key == ord(self.CHANGE_MASK.upper()):
                self.active_mask = self.grabcut_mask if self.active_mask is self.mrnn_mask else self.mrnn_mask
            elif key == ord(self.SELECT_MASK) or key == ord(self.SELECT_MASK.upper()):
